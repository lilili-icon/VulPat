...
def selectData(path,list,key):
    csv_result = pd.read_csv(path)
    data = csv_result[csv_result["ID"].isin(list)]
    data.to_csv("output/CWE-119"+str(key)+".txt")
    return data


if __name__ == '__main__':
    path = "embedding/CWE-119.csv"
    data,target = load_data(path)
    xmeans_result = myxmeans(data, target, 10, 20)

    for key in xmeans_result['clusters'].keys():
        list = xmeans_result['clusters'][key]['target']
        path = 'data/CWE-csv/4080-CWE-119.csv'
        selectData(path, list,key)
